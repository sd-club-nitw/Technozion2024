...
const AuthProvider = ({ children }) => {
  ...
  const register = async (registrationData) => {
    ...
    try {
      ...
      const emailDomain = registrationData.email
        ?.trim()
        .toLowerCase()
        .split('@')[1];
      ...
      if (res.ok) {
        ...
        setUser(data.user);
        alert(
          `Successfully Registered. you id is ${
            data?.user?.registrationNum || 'NA'
          }`
        );
        navigate('/');
      } else {
  ...
  return (
    <AuthContext.Provider
      value={{ user, login, register, logout, loading, setLoading }}
    >
