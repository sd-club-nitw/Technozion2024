...
// Missing helper components / icons
...
const Card = () => {
  ...
  // Helper to render the tabbed content
  const renderContent = () => {
    if (activeTab === 'overview') {
      return (
        <>
          <section className="flex flex-col gap-y-1 items-start justify-start">
            <span className="opacity-70 text-[1rem]">Description</span>
            <div className="lg:text-[1.1rem]">{overview?.description}</div>
          </section>
          {total_cost ? (
            <section className="flex flex-col gap-y-1 items-start justify-start">
              <span className="opacity-70 text-[1rem]">Prizes worth</span>
              <span className="text-[1.2rem]">₹ <strong>{total_cost}</strong></span>
              <small className="opacity-60">prizes will be given based on judging criteria</small>
            </section>
          ) : null}
          {overview?.cash_prize && (
            <section className="flex flex-col gap-y-1 items-start justify-start">
              ...
              <small className="opacity-60">cash prize will be given based on performance</small>
            </section>
          )}
          <section className="flex flex-col gap-y-1 items-start justify-start">
            <span className="opacity-70 text-[1rem]">Participation</span>
            <div className="font-bold">{overview?.team_size === 1 ? `Individual` : overview?.team_size}</div>
          </section>
          <section className="flex flex-col gap-y-1 items-start justify-start">
                ...
                <div key={index} className="flex flex-col mb-5 bg-gray p-3 rounded-md lg:min-w-[15em]">
                  ...
                  {contact?.phone && (
                    <span className="flex justify-between gap-x-3 items-center cursor-pointer">
                      <span className="flex gap-x-3 items-center">+91 {contact.phone}</span>
    ...
    if (activeTab === 'rules') {
      return (
        <div>
          <div className="font-bold text-2xl uppercase w-full ">Rules</div>
    ...
    if (activeTab === 'judging_criteria') {
      return (
        <div>
          <div className="font-bold text-2xl uppercase w-full ">Judging Criteria</div>
          <div className="flex flex-col gap-y-3 mt-5">
            {Array.isArray(judging_criteria)
              ? judging_criteria.map((jc, idx) => (
                  <div key={idx} className="flex items-start gap-x-3">
                    <span className="font-bold">{idx + 1}.</span>
                    <span>{jc}</span>
                  </div>
                ))
              : typeof judging_criteria === 'string'
              ? <div className="mt-2">{judging_criteria}</div>
              : <div className="mt-2">No judging criteria provided.</div>
            }
          </div>
        </div>
      );
        ...
        <div className="text">
          <div className="logo-cardnav-container">
            ...
            <div className="content-container">
              {title && <h2 className="event-title font-bold text-xl mb-3">{title}</h2>}
              <div className="cardnav">
                ...
                {judging_criteria && (
                  <button onClick={() => setActiveTab('judging_criteria')} className={activeTab === 'judging_criteria' ? 'active' : ''}>Judging Criteria</button>
                )}
                <button className="back-button" onClick={handleBack}>Back</button>
              </div>

              <div className="content" ref={contentRef} key={activeTab}>{renderContent()}</div>
